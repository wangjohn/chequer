...
var (
	sobelX = &graphics.SeparableKernel{
		X: []float64{-1, 0, +1},
		Y: []float64{1, 2, 1},
	}
	sobelY = &graphics.SeparableKernel{
		X: []float64{1, 2, 1},
		Y: []float64{-1, 0, +1},
	}
	scharrX = &graphics.SeparableKernel{
		X: []float64{-1, 0, +1},
		Y: []float64{3, 10, 3},
	}
	scharrY = &graphics.SeparableKernel{
		X: []float64{3, 10, 3},
		Y: []float64{-1, 0, +1},
	}
	prewittX = &graphics.SeparableKernel{
		X: []float64{-1, 0, +1},
		Y: []float64{1, 1, 1},
	}
	prewittY = &graphics.SeparableKernel{
		X: []float64{1, 1, 1},
		Y: []float64{-1, 0, +1},
	}
)

func diffOp(mag, dir *image.Gray, src image.Image, opX, opY *graphics.SeparableKernel) error {
	if src == nil {
	...
	mx := image.NewGray(b)
	if err := graphics.Convolve(mx, srcg, opX); err != nil {
		return err
	}

	my := image.NewGray(b)
	if err := graphics.Convolve(my, srcg, opY); err != nil {
		return err
	}
